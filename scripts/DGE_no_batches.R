...
# ZP
# 4/23/25
# purpose: DGE without batch effect, still subsetting by comparison type but not by experiment
# pulling differentially expressed genes, up and down regulated
...
residuals <- read.table("GE_matrix_corrected_experiment_no_batch_effect_logCPM_voomed.txt", 
...
hgnc_symbols <- rownames(residuals)  # or residuals_df$hgnc_symbol
print(head(hgnc_symbols))

#   
  # Step 2: Subset columns that match row names of meta_exp
  matching_samples <- intersect(colnames(residuals), rownames(meta_data))
...
#   # now we subset on comparisons
  for (comp_name in names(comparisons)) {
    ...
    design <- model.matrix(~ 0 + group, data = meta_comp) 
    cat("\nDesign matrix for", comp_name, ":\n")
    print(head(design))
    
    
    fit <- lmFit(filtered_residuals[, rownames(meta_comp)], design) %>%
      eBayes()
    ...
    cat("Samples in comparison", comp_name, ":", nrow(meta_comp), "\n")
    print(table(meta_comp$group))
    ...
    # print(fit$p.value[, coef_name])
    
    # results$experiment <- paste0("Exp", exp)
    results$comparison <- comp_name
    all_results[[paste0(comp_name, "_no_batch_effect")]] <- results
    
    # Store counts
    sig_count <- sum(results$significant, na.rm = TRUE)
    significant_counts[[paste0(comp_name, "_no_batch_effect")]] <- sig_count
    
    # Save results
    write.csv(
      results,
      file = paste0(results_dir, "DGE", comp_name, "_no_batch_effect_fdr_", fdr_threshold, ".csv"),
      row.names = FALSE)
