...
qpcr_data <- read.csv("~/Desktop/CU_coding/qPCR/raw_data/Exp16_6.7.24_u4cs_20240607_170611_Results_20240607 184111.csv", 
                      skip = skip_meta, 
                      stringsAsFactors = FALSE) # put in path to YOUR data

my_cols <- c("#648FFF", "#785EF0", "#DC267F", "#FE6100", "#FFB000", "darkolivegreen2", ) # should be sample length as probe_list
...
rel_conc_bar_order <- rel_conc_bar + 
  ...
  # geom_hline(yintercept = 1.328, linetype = 'dashed', color = 'black') +  # Add dotted line
  geom_hline(yintercept = 211, linetype = 'dashed', color = 'black')  # Add dotted line
rel_conc_bar_order

# run 2x?
rel_conc_bar_order <- rel_conc_bar_order + 
  # geom_text(aes(x = 3.5, y = 1.4, label = "Lower Limit Mild GOF"), 
  #           hjust = 0, nudge_x = 5, vjust = 0, size = 3) +
  geom_text(aes(x = 0.45, y = 214, label = "Upper Limit Strong GOF"), 
