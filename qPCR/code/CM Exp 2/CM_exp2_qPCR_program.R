...
qpcr_data <- read.csv("~/Desktop/CU_coding/qPCR/raw_data/Exp2_CM_Exp24_ZP_8.23.2024_U4Cs_real_20240823_152858_Results_20240823 170234.csv", 
                      skip = skip_meta, 
...
qpcr_data <- qpcr_data %>%
  filter(grepl("CM$", Sample)) %>%
  mutate(Sample = sub(" CM$", "", Sample))

## RESTART HERE FOR FURTHER ANALYSIS
...
# Group by Sample and calculate the average Cq
ref_data$ref_Cq <- as.numeric(as.character(ref_data$ref_Cq))
ref_data <- ref_data %>%
  filter(ref_target != is.na(ref_target) & ref_target != "") %>%
  group_by(Sample, ref_target) %>%
  summarize(ref_Cq = mean(ref_Cq, na.rm = TRUE), .groups = 'drop')
...
rel_conc_bar_order <- rel_conc_bar + 
  scale_x_discrete(labels = c(
    ...
    "pTRIP JAK1 R1041Q" = "R1041Q",
    "pTRIP JAK1 R893S" = "R893S"
  ), 
