...
qpcr_data <- read.csv("~/Desktop/CU_coding/qPCR/raw_data/Exp33_11.20.24_U4Cs_upsidedown_real_20241120_121128_Results_20241120 134613.csv", 
                      skip = skip_meta, 
...
# Local variables:
probe = "ISG15" # list of probes of interest
...
avg_cq_values <- qpcr_data %>%
  ...
  filter(!str_detect(Sample, "^pTRIP JAK1 I359M 2$"))
...
qpcr_data %>%
  filter(Sample == "pTRIP JAK1 I359M 2")

qpcr_data <- qpcr_data %>%
  mutate(Sample = str_to_lower(str_trim(Sample)))
